...
public class Fruit_IceCreamExplosiveTileCombineHandler extends Fruit_IceCreamCombineHandler implements TimerEvent.TimerEventListener {
    ...
    // Constructors
    //--------------------------------------------------------
    public Fruit_IceCreamExplosiveTileCombineHandler(Engine engine) {
        super(engine);
        ...
        for (int i = 0; i < row; i++) {
            for (int j = 0; j < col; j++) {
                ...
                if (t.getTileType() == targetType && t.getTileState() == Fruit_TileState.IDLE) {
                    ...
                    t.setSpecialType(Fruit_SpecialType.EXPLOSIVE);
                    mSpecialTiles.add(t);
